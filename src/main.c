...
void scan_r(char *ip,int from_port,int end_port,int *result);
void thread_run(void *arg);
void mulite_thread_run(char *ip,int from_port,int end_port,int thread_count);
...
{
    if (argc < 4) {
        printf("port_scan ${ip} ${port} ${start port} ${end port}\n");
        return 0;
    }
    ...
    if(argc == 5)
    {
        ...
        res = (int **)malloc(sizeof(int*)*thread_count);
        mulite_thread_run(ip, from_port, end_port,thread_count);
    }
    else{
        //默认使用10线程加载
        int thread_count = 10;
...
void thread_run(void *arg)
{
    ...
    int size = arg_thread1 -> end_port - arg_thread1->from_port + 2;
    int *result = (int *)malloc(sizeof(int)*size);
    scan_r(arg_thread1->ip, arg_thread1->from_port, arg_thread1->end_port, result);
